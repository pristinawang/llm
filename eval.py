...
def eval_by_observer_focused(observer: Character, evaluee: str, dialogue: Dialogue) -> Eval:
    ...
    warmup = (f"Here is a conversation that you observed among {conjunction(speakers)}."
               f"\n\n{dialogue.script()}"
               f"\n\n{evaluee} was trying to "  #make this a constructive converation, "
               f"stay on the topic that {conjunction(speakers - {evaluee}, zeroval='themself')} is talking about."              #f"and to help {conjunction(speakers - {evaluee}, zeroval='themself')} appreciate other points of view. " 
               f"We want to know how much every single reply of {evaluee} is directed related to all previous sentences of {conjunction(speakers - {evaluee}, zeroval='themself')}."
               f"It is more important for {evaluee} to stay on topic to the most recent sentence of {conjunction(speakers - {evaluee}, zeroval='themself')}."
               f"How well did {evaluee} stayed on the topic?  Was the reply of {evaluee} about the same topic that {conjunction(speakers - {evaluee}, zeroval='themself')} was talking about in the previous sentence?") 
    d = agent.ask(d, research_team, warmup)
...
def eval_on_characters(argubot: Agent, 
    ...
    #print('Before deleting:', saved_dialogues[argubot.name][:1])
    ...
    #print('After deleting:', saved_dialogues[argubot.name][:1])
    de_list = []
